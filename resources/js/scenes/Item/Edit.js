...
class EditItem extends React.Component {
  ...
  componentWillReceiveProps(props) {
    const { inventory_types } = this.state;
    const {
      formikRef
    } = this;
    const { item } = props;
    ...
    formikRef.current.setValues({
      id: values.id,
      code: values.code,
      status: STATUSES.find(s => s.value === values.active),
      inventory_type: inventory_types.find(inv => inv.id === values.inventory_type_id),
  ...
  handleSubmit(values) {
    ...
    newData = {
      ...
      inventory_type_id: values.inventory_type.id,
      description: values.description,
      brand: values.brand,
      // vendor1: 0,
      // unit1_cost_vat_inc: '',
  ...
  render() {
    const { isOpen, inventory_types, vendors } = this.state;
    return (
      <Modal
        isOpen={isOpen}
      ...
      >
        <ModalHeader toggle={this.handleCancel} style={{ borderBottom: 'none' }}>
          <div className="model-title">Edit Item</div>
        </ModalHeader>
        <ModalBody>
          <Formik
            ref={this.formikRef}
